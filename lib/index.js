            ...
            Program: {
                ...
                exit: function exit(path, state) {
                    ...
                    if (opts.messagesDir && descriptors.length > 0) {
                        (function () {
                            var messagesFilename = p.join(opts.messagesDir, p.dirname(p.relative(process.cwd(), filename)), basename + '.properties');

                            (0, _mkdirp.sync)(p.dirname(messagesFilename));

                            descriptors.forEach(function (descriptor) {
                                var buffer = ['# ' + descriptor.description, descriptor.id + '=' + descriptor.defaultMessage, '\n'].join('\n');

                                (0, _fs.appendFileSync)(messagesFilename, buffer);
                            });
                        })();
                    }
                }
